...
namespace app\modules\frontend\models\_print;
...
class Record extends RecordModel
{
    ...
    {
        return [
            [['state','id', 'infraction_date', 'open_date', 'state_id', 'user_id', 'ticket_fee', 'ticket_payment_expire_date', 'status_id'], 'integer'],
    ...
    public function searchPrint($params)
    {
        ...
        $query->andFilterWhere( ['in', 'status_id', [3020, 3021, 4030, 4060]]);


        return $dataProvider;
    }

    public function searchPrintlast($params)
    {
        $query = $this->find()
            ->select([
                'id' => 'record.id',
...
//        $dataProvider->getSort()->attributes['address'] = [
    ...
    public function searchPrintList($params)
    {
        $query = $this->find()
            ...
            ->joinWith([
                'user' => function ($query) {
                    $query->from('User user');
                    //  $query->join('Owner owner');
                    //$query->join('JOIN', 'Owner', 'Record.user_id = Owner.id');
                },
